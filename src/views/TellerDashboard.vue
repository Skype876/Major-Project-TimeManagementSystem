<template>
  <div class="flex min-h-screen flex-col">
      ...
      <div class="container mx-auto flex h-16 items-center justify-between py-4">
        ...
        <div class="flex items-center gap-4">
          <button
            ...
            <span
              class="absolute -top-1 -right-1 inline-flex items-center justify-center h-5 w-5 text-xs font-bold text-black bg-white rounded-full border border-black">3</span>
          </button>
          <div class="relative">
    ...
    <main class="flex-1 flex justify-center items-center">
      <div class="container py-8 max-w-screen-lg">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div class="md:col-span-2 space-y-6">
            <div v-if="currentStudent.studentId != null" class="rounded-lg border bg-card text-card-foreground shadow-sm">
            ...
            <div class="rounded-lg border bg-card text-card-foreground shadow-sm ml-2">
              ...
              <div class="p-6">
                <TellerQueueTable @student-info="updateCurrentStudent" />
              </div>
            </div>
          </div>
...
const resetCurrentStudent = () => {
  Object.keys(currentStudent).forEach(key => {
    currentStudent[key] = key === 'email' || key === 'phone' ? 'N/A' : null
  })
}
...
const updateCurrentStudent = (studentData) => {
  ...
  console.debug('Current student updated:', JSON.stringify(currentStudent))
}

const handleServiceAction = async (action, errorMessage) => {
  if (!currentStudent.studentId) {
    console.warn('No student selected for action:', action)
    return false
  }
...
const handleCompleteService = async () => {
  const success = await handleServiceAction(
    'complete',
    'Failed to complete student service'
  )
...
const handleRemoveService = async () => {
  const success = await handleServiceAction(
    'remove',
    'Failed to remove student from queue'
  )
